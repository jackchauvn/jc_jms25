...
/*-------------------------------------------------------------------------------
# plg_jmscomponent - JMS Membership Sites Plugin
# -------------------------------------------------------------------------------
# author    			Infoweblink
...
class plgSystemJmscomponent extends JPlugin
{
	...
	function jmsRestrictCom() {
		...
		if ($cnf->get('custom_registration_page')) {
			$mainframe->redirect("index.php?option=com_content&view=article&id=" . $cnf->get('custom_registration_page') . "&Itemid=".$Itemid);
		} else {
