...
class MediaControllerFolder extends JControllerLegacy
{
	...
	public function delete()
	{
		...
		$redirect = 'index.php?option=com_jcmedia&folder=' . $folder;
		if ($tmpl == 'component') {
		...
		if (!$user->authorise('core.delete', 'com_jcmedia'))
		{
		...
		if (count($paths))
		{
			foreach ($paths as $path)
			{
				...
				if (is_file($fullPath))
				{
					...
					$result = $dispatcher->trigger('onContentBeforeDelete', array('com_jcmedia.file', &$object_file));
					...
					$dispatcher->trigger('onContentAfterDelete', array('com_jcmedia.file', &$object_file));
				...
				elseif (is_dir($fullPath))
				{
					...
					{
						// Trigger the onContentBeforeDelete event.
						$result = $dispatcher->trigger('onContentBeforeDelete', array('com_jcmedia.folder', &$object_file));
						if (in_array(false, $result, true)) {
							// There are some errors in the plugins
							JError::raiseWarning(100, JText::plural('COM_MEDIA_ERROR_BEFORE_DELETE', count($errors = $object_file->getErrors()), implode('<br />', $errors)));
							continue;
						...
						// Trigger the onContentAfterDelete event.
						$dispatcher->trigger('onContentAfterDelete', array('com_jcmedia.folder', &$object_file));
						$this->setMessage(JText::sprintf('COM_MEDIA_DELETE_COMPLETE', substr($fullPath, strlen(COM_MEDIA_BASE))));
					}
					else
					{
	...
	public function create()
	{
		...
		$this->setRedirect('index.php?option=com_jcmedia&folder='.$parent.'&tmpl='.JRequest::getCmd('tmpl', 'index'));

		if (strlen($folder) > 0)
		{
			if (!$user->authorise('core.create', 'com_jcmedia'))
			{
				// User is not authorised to delete
			...
			if (!is_dir($path) && !is_file($path))
			{
				...
				JPluginHelper::importPlugin('content');
				$dispatcher	= JDispatcher::getInstance();
				$result = $dispatcher->trigger('onContentBeforeSave', array('com_jcmedia.folder', &$object_file));
				if (in_array(false, $result, true))
				{
					// There are some errors in the plugins
				...
				$dispatcher->trigger('onContentAfterSave', array('com_jcmedia.folder', &$object_file, true));
