...
/*-------------------------------------------------------------------------------
# plg_jmscontent - JMS Membership Sites Plugin
# -------------------------------------------------------------------------------
# author    			Infoweblink
...
class plgContentJmscontent extends JPlugin {
	...
	function onContentBeforeDisplay($context, &$row, &$params, $page=0) {		
		...
		if($url == $current) {
			...
			$session->clear('jms_article_id');
		}
		
		if ($cnf->get('se')) {
			$bots = explode("|", $cnf->get('bots'));
			foreach($bots as $bot)
		...
		// show introtext only if {INTRO} tag found and user is not a subscriber yet
		if (preg_match("/{INTRO}/", $row->text)) {
			
			$subscr = jmsGetSubscr('article', $userid, $db);
			$session_introtext = getArticleIntroText($session->get('jms_article_id'));
			
			if ($subscr) {
				$row->text = $row->fulltext;
				//$row->text = preg_replace("/{INTRO}/", '', $row->text);
			} else {
				//$row->text = $row->introtext;
				$row->text = preg_replace("/{INTRO}/", $session_introtext, $row->text);
				return;
			}
		}
	...
	function jmsMarkTitle(&$row, $param) {
		...
		if ($param->def('link') == 1) {
			...
			if ($param->get('custom_registration_page')) {
				$img = sprintf('<a href="%s">%s</a>', JRoute::_('index.php?option=com_content&view=article&id=' . $param->get('custom_registration_page') . '&Itemid=' .$Itemid), $img);
			} else {
		...
		switch ($param->def('mark_type')) {
			...
			case 2 :
				...
				if ($row->introtext) {
					$row->introtext = $img . ' ' . $row->introtext;
				} else {
					$row->title = $img . ' ' . $row->title;
				}
				break;
	...
	// Get restricted page's title and introtext
	function getArticleIntroText($id) {
